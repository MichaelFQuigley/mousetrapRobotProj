...
# trackbars for dilation and erosion
cv2.createTrackbar('Y Dilation', windowName, 1, 100, scrollBarCallback)
cv2.createTrackbar('X Dilation', windowName, 1, 100, scrollBarCallback)
cv2.createTrackbar('Y Erosion', windowName, 1, 100, scrollBarCallback)
cv2.createTrackbar('X Erosion', windowName, 1, 100, scrollBarCallback)
...
def display():
    ...
    dilation = (cv2.getTrackbarPos('Y Dilation', windowName), cv2.getTrackbarPos('X Dilation', windowName))
    erosion = (cv2.getTrackbarPos('Y Erosion', windowName), cv2.getTrackbarPos('X Erosion', windowName))

    img2 = objectDetection.filter(img, (bMin, gMin, rMin), (bMax, gMax, rMax))
    img3 = objectDetection.postProcess(img2, dilation, erosion)
